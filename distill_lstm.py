...
def distill_lstm(
    ...
    for epoch in range(epochs):
        ...
        for i,(x_t,y_t,t_t) in enumerate(train_loader):
            ...
            distillation_loss = loss_fn(pred,t_t.unsqueeze(1).to(device)) * alpha
            ground_truth_loss = loss_fn(pred,y_t.unsqueeze(1).to(device)) * beta
        ...
        with torch.no_grad():
            for i,(x_v,y_v) in enumerate(val_loader):
                ...
                val_loss = loss_fn(val_pred, y_v.unsqueeze(1).to(device))
